...
classes2 = {0:zero, 1:zero, 2:zero, 3:zero, 4:zero, 5:zero, 6:zero, 7:zero, 8:zero, 9:zero}
...
for test in Test_data:
    ...
    for j in range(len(Train_label)):
        ...
        if tmp < max(ref['data']):
            need = ref['data'].index(max(ref['data']))
            
            ref['data'][need] = tmp
            ref['label'][need] = Train_label[j][0]
    for j in range(10):
        res.append(ref['label'].count(j))
    ...
    estimated.append(maxi)
    confirmArray[Test_label[i][0]][maxi].append(confidence(res))
    i += 1
