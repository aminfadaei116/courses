...
def confidence(res):
    res = sorted(res)
    return (res[0]**(-2) - res[1]**(-2))/(res[0]**(-2))
    return 1 - exp(data_dim * ( -log(res[1]) + log(res[0]) ) )
...
for test in Test_data:
    ...
    for j in range(len(Train_label)):
        classes[Train_label[j][0]].append(distance(test, Train_data[j]))
    res = []
    for j in range(10):
        classes[j].sort()
        res.append(classes[j][k-1])
    estimate = res.index(min(res))
