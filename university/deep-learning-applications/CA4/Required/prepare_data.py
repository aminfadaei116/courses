...
import json
from object_detection.utils.data_loader import SpectrogramDataset, AudioDataLoader, BucketingSampler
			...
			if end_file=='wav':
				for sound in glob(file+'/*'):
					...
					if sfn in validation_file_names:
						data_val_manifest.write('./data/val/wav/'+sfn+'.wav'+', ./data/val/txt/'+sfn+'.txt\n')
						shutil.copyfile(sound, './data/val/wav/'+sound_name) 
						
					elif sfn in test_file_names:
						data_test_manifest.write('./data/test/wav/'+sfn+'.wav'+',./data/test/txt/'+sfn+'.txt\n')
						shutil.copyfile(sound, './data/test/wav/'+sound_name) 
						
					elif sfn in train_file_names:
						data_train_manifest.write('./data/train/wav/'+sfn+'.wav'+',./data/train/txt/'+sfn+'.txt\n')
